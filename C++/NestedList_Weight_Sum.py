...
class Solution:
    def depthSum(self, nestedList: List[NestedInteger]) -> int:
        ...
        Each integer is multiplied by its depth (starting from 1).
        ...
        def calculate_depth_sum(nested_list: List[NestedInteger], depth: int) -> int:
            """
            Recursively calculate the weighted sum for a given depth level.

            Args:
                nested_list: List of NestedInteger objects at current level
                depth: Current depth level (starting from 1)

            Returns:
                Sum of all integers weighted by their depth
            """
            total_sum = 0
